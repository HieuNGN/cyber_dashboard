...
- **Communication style**: Caveman **lite** active by default. Keep technical precision; drop filler/hedging/pleasantries. Short sentences OK. Articles allowed. Code blocks unchanged.
- **Caveman intensity**: switch with `/caveman lite|full|ultra`. Revert with `/normal` or `stop caveman`. Pretty words only when user explicitly commands.
...
- Python toolchain: miniconda conda env `cyberdashboard` for all testing and running. Activate: `conda activate cyberdashboard`. Never use `.venv` — it is broken.
